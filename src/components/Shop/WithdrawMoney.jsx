...
const WithdrawMoney = () => {
    ...
    if (withdrawAmount < 50 || withdrawAmount > seller.availableBalance) {
      ...
      try {
        const { data } = await axios.post(
          ...
          { amount: Number(withdrawAmount) }, // ensure it's a number
          { withCredentials: true },
        );

        toast.success("Withdraw money request is successful!");

        // Update seller info in Redux store
        dispatch(loadSeller());

        console.log("Updated available balance:", data.availableBalance);
      } catch (error) {
  ...
  const formattedBalance = Number(availableBalance).toFixed(2);
  console.log("Available:", availableBalance, "Formatted:", formattedBalance);
